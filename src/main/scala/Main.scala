...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(sessionizedDf.count)
    sessionizedDf.sort(desc(SESSION_DURATION)).show(10, false)
  }
  ...
  def sessionize(df: DataFrame): DataFrame = {
    ...
    val dfWithSessionId = df
      ...
      .withColumn(IS_NEW_SESSION_COL,
        (col(TIMESTAMP_LONG_COL) - coalesce(lag(col(TIMESTAMP_LONG_COL), 1).over(timeOrderedWin), lit(0)) > INACTIVE_TIME_THRESHOLD_MIN * 60).cast("long"))
